...
module.exports = {
  ...
  run: async (message) => {
    //channels excluded
    if (
      message.channel.id == "764539644335358003" ||
      message.channel.id == "764556340425130005" ||
      message.channel.id == "767151113421979678"
    )
      return;
    ...
    if (isValidURL(message.content.toLowerCase()) == true) {
      let storyfetcher = `https://api.smmry.com/&SM_API_KEY=${
        process.env.TLDR
      }&SM_URL=${message.content.toLowerCase()}`;
      ...
      await message
        .awaitReactions(newfilter, { max: 1, time: 300000, errors: ["time"] })
        .then(async (collected) => {
          const waitreaction = collected.first();
          var reacted_user = collected
            .first()
            .users.cache.map(
          ...
          if (waitreaction.emoji.name === "❓") {
            ...
            if (data.sm_api_character_count > 2048) {
              ...
              const embed = new Discord.MessageEmbed()
                ...
                .setFooter(
                  `Click on the forward button to go to the next page.\nPage [1/2]\nRequested by ${reacted_user[1]}`
                );
                  ...
                  .then((collected) => {
                    ...
                    if (reaction.emoji.name === "⏩") {
                      const editembed = new Discord.MessageEmbed()
                        ...
                        .setColor("#FF7F50")
                        .setDescription(
                          data.sm_api_content.substring(
                            2043,
                            data.sm_api_content.length
                          )
                        )
                        .setFooter(
                          `I have reduced the article for you by ${data.sm_api_content_reduced}\nPage[2/2]\nRequested by ${reacted_user[1]}`
                        );
